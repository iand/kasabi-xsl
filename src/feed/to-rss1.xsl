...
<xsl:transform 
	...
	<xsl:template match="/">
	   <rdf:RDF>
	      <xsl:apply-templates mode="root" select="/sparql:sparql/*"/>
	   </rdf:RDF>
	</xsl:template>

	<!-- Ignore head element -->
	<xsl:template mode="root" match="sparql:head"/>

	<!-- Process results element -->
	<xsl:template mode="root" match="sparql:results[sparql:result/sparql:binding/sparql:uri]">
	   ...
	   <rss:channel rdf:about="">
	      ...
	      <rss:items>
	         <rdf:Seq>
	            <xsl:apply-templates mode="result-sequence" select="sparql:result[sparql:binding[@name=$linkname and sparql:uri]]"/>
	         </rdf:Seq>
	      </rss:items>
	   </rss:channel>
	   <!-- Create each item, only the ones with an rss:link binding -->
	   <xsl:apply-templates mode="result-item" select="sparql:result[sparql:binding[@name=$linkname and sparql:uri]]">
	...
	<xsl:template mode="linkname" match="*">
	   <xsl:choose>
	      <xsl:when test="sparql:result/sparql:binding[@name='rsslink' and sparql:uri]">
	         <xsl:text>rsslink</xsl:text>
	      </xsl:when>
	      <xsl:otherwise>
	         <xsl:value-of select="sparql:result/sparql:binding[sparql:uri][1]/@name"/>
	      </xsl:otherwise>
	   </xsl:choose>
	</xsl:template>
	...
	<xsl:template mode="titlename" match="*">
	   <xsl:choose>
	      <xsl:when test="sparql:result/sparql:binding[@name='rsstitle' and sparql:literal]">
	         <xsl:text>rsstitle</xsl:text>
	      </xsl:when>
	      <xsl:otherwise>
	         <xsl:value-of select="sparql:result/sparql:binding[sparql:literal][1]/@name"/>
	      </xsl:otherwise>
	   </xsl:choose>
	</xsl:template>
	...
	<xsl:template mode="result-item" match="*">
	   ...
	   <rss:item rdf:about="#{generate-id()}{$_id}">
	      <rss:link>
	         <xsl:apply-templates mode="value" select="sparql:binding[@name=$linkname]"/>
	      </rss:link>
	      <rss:title>
	         <xsl:apply-templates mode="value" select="sparql:binding[@name=$titlename]"/>
	      </rss:title>
	      <!-- Generate description from other non-link/title bindings -->
	      <xsl:variable name="description">
	         <xsl:for-each select="sparql:binding[@name!=$linkname and @name!=$titlename]">
	...
	<xsl:template mode="value" match="sparql:binding">
	   <xsl:choose>
	      ...
	      <xsl:when test="sparql:bnode">
	         <xsl:text>(</xsl:text>
	         <xsl:value-of select="sparql:bnode"/>
	         <xsl:text>)</xsl:text>
	      </xsl:when>
	      <xsl:when test="sparql:literal[@datatype]">
	         <xsl:value-of select="sparql:literal/text()"/>
	         <xsl:text> ^^ </xsl:text>
	         <xsl:value-of select="sparql:literal/@datatype"/>
	      </xsl:when>
	      <xsl:when test="sparql:literal[@xml:lang]">
	         <xsl:value-of select="sparql:literal/text()"/>
	         <xsl:text> @ </xsl:text>
	         <xsl:value-of select="sparql:literal/@xml:lang"/>
	      </xsl:when>
